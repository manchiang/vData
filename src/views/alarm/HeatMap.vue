...
export default {
  ...
  mounted() {
    ...
    this.$Event.$on("getalarmHotData", (res) => {
      if (res) {
        if (mapView) {
          mapView.destroy();
          mapView = null;
  ...
  beforeDestroy() {
    this.$Event.$off("getalarmHotData"); //销毁
  },
  methods: {
    async getHotData(storeCurr) {
      let [err, res] = await this.api_post(
        "warningMonitoring/select/selectCabinetWarningCount",
        {
    ...
    initMap(heatData) {
      let firstData = heatData[0];
      let center = new TMap.LatLng(firstData.lat, firstData.lng);
      //初始化地图
      mapView = new TMap.Map(document.getElementById("heat_container2"), {
        zoom: 11.7, //设置地图缩放级别
      ...
      heatView.setData(heatData); //设置数据
    },
  },
};
</script>
